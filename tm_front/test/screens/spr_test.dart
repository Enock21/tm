...
void main() {
  group('Tela de Recuperação de Senha - Validações', () {
    testWidgets('SPR-TST1: Exibe erro quando o formato do e-mail é inválido',
        (tester) async {
    ...
    testWidgets('SPR-TST2: Exibe erro quando o campo de e-mail está vazio',
        (tester) async {
      await tester.pumpWidget(const MaterialApp(home: SPRMain()));
    ...
    testWidgets('SPR-TST3: Exibe erro para email não encontrado',
        (tester) async {
      await tester.pumpWidget(const MaterialApp(home: SPRMain()));

      await tester.enterText(
          find.byType(TextFormField).first, 'naoexiste@email.com');
      await tester.tap(find.text('Enviar'));
      await tester.pumpAndSettle();

      expect(find.text(ErrorMsgs.emailNotFound), findsOneWidget);
    });

    testWidgets('SPR-TST4: Exibe feedback positivo para email válido',
        (tester) async {
      await tester.pumpWidget(const MaterialApp(home: SPRMain()));
    ...
    testWidgets('SPR-TST5: Não exibe erro para email válido', (tester) async {
      ...
      expect(find.text(ErrorMsgs.emailNotFound), findsNothing);
    });
  });
  ...
  group('Tela de Alteração de Senha - Validações', () {
    ...
    testWidgets('SPR-TST9: Erro de senhas diferentes', (tester) async {
      ...
      expect(find.text(ErrorMsgs.passwordMismatch), findsOneWidget);
    });

    testWidgets(
        'SPR-TST10: Exibe feedback positivo para alteração realizada com êxito',
        (tester) async {
      await tester.pumpWidget(const MaterialApp(home: SPRAlteration()));
    ...
    testWidgets('SPR-TST11: Não exibe erro para alteração realizada com êxito',
        (tester) async {
      await tester.pumpWidget(const MaterialApp(home: SPRAlteration()));
      ...
      expect(find.text(ErrorMsgs.passwordMismatch), findsNothing);
    });
  });
}
